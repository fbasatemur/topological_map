...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "topological_map_node"); /// initializes the ros node with default name
    ros::NodeHandle n;
...
namespace tuw_graph
{
    VoronoiGeneratorNode::VoronoiGeneratorNode(ros::NodeHandle &n) : voronoi_map::VoronoiPathGenerator(), VoronoiGraphGenerator(), Serializer(), n_(n), n_param_("~")
    {
        ...
        voronoi_map_pub = n.advertise<sensor_msgs::Image>("voronoi_mapping_node", 1);
    ...
    void VoronoiGeneratorNode::createGraph(const nav_msgs::OccupancyGrid::ConstPtr &_map, size_t _map_hash)
    {
        ...
        // cv::imwrite("/mnt/shared_folder/voronoiMap_.jpg", voronoiMap_);
        publishVoronoiMap(voronoiMap_);
